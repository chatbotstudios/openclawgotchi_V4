...
async def run_cron_job(job):
    ...
    if internal_reminder:
        ...
        cron_system = (
            ...
            "Reply with ONE short, friendly message to send them right now. "
...
def main():
    ...
    load_all_extensions(extensions_path)
    _, schemas = get_tools_and_schemas()
    log.info(f"Loaded {len(schemas) if schemas else 0} tool schemas from registry")
    
    # Register mission listeners so @hook decorators are activated
    try:
        import core.missions.listeners
    except Exception as e:
        log.warning(f"Failed to load mission listeners: {e}")
        
    # Discover and load hooks
    discover_and_load_hooks()
    
    # Initial screen will be handled by discord_bot.py on_ready
    
    # Fire startup hook
    run_hook(HookEvent(event_type="startup"))
    
    # Start the background face animation engine
    ...
    if os.environ.get("GOTCHI_DEPLOYMENT", "Device") == "Local":
        try:
            from ui.web_dash import start_web_server
            start_web_server()
        except Exception as e:
            log.error(f"Failed to start localhost live dashboard: {e}")
    else:
    ...
    if BOT_PLATFORM == "discord":
        ...
        log.info("Cooldown: waiting 5s before Discord handshake...")
        import time
        time.sleep(5)
        run_discord()
    else:
        # Check token
        if not BOT_TOKEN:
            log.warning("TELEGRAM_BOT_TOKEN not set! Suspending main loop until setup.")
            show_face(mood="boot", text="Run 'gotchi setup'")
            import time
            while True:
                time.sleep(3600)
        ...
        try:
            ...
            pass # Ignore if imports are missing for some handlers
