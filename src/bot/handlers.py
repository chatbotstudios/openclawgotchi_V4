...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Hi {user.first_name}! I'm 🦋OpenClawGotchi🦋, your edge AI companion.\n\n"
        f"*Commands:*\n"
        f"/status — system & XP\n"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conv_id = chat.id
    router = get_router()
    mode = "L" if router.force_lite else "P"
    
    report = get_status_report(conv_id)
    msg = format_status_markdown(report)
...
async def cmd_pro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _, is_lite = set_llm_mode(requested)
    ...
    if is_lite:
        ...
        await update.message.reply_text("🧠 Mode: Pro (Advanced Logic) — Smart & Heavy")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from core import litellm_connector
...
async def cmd_health(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from config import DB_PATH, SRC_DIR
    from db.stats import get_stats_summary
